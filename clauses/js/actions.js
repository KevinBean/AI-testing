...
let selectedContentItems = []; // Declare the array to store selected content items
...
function initializeActionButtons() {
  ...
  $("#runActionBtn").off("click").on("click", async function() {
    ...
    try {
      console.log("Selected content items before assembly:", selectedContentItems);
      const contentText = await assembleContentFromSelectedItems();
      //log the content text for debugging
      console.log("Assembled content text:", contentText);
  ...
  $("#confirmContentSelection").off("click").on("click", function() {
    ...
    console.log("Selected content items:", selectedContentItems);
        ...
        $(".block-selection-list .select-content-item").on("change", function() {
          ...
          if ($(this).prop("checked")) {
            ...
            selectedContentItems.push({
              ...
              type: "block",
              content: blockDiv.find("label p").text()
...
async function assembleContentFromSelectedItems() {
  ...
  console.log("Assembling content from selected items:", selectedContentItems);
          ...
          store.get(parseInt(item.id)).onsuccess = function(e) {
            ...
            if (block) {
              ...
              resolve({
                ...
                content: `Block with ID ${item.id} not found`,
                error: true
              });
            }
          };
  ...
  if (contentParts.length === 0) {
    ...
    console.log("Using input content instead:", inputContent);
    if (inputContent) {
      contentParts.push(inputContent);
    } else {
      console.log("No content was selected or provided");
      return "No content was selected for processing.";
    }
  }
  
  const assembledContent = contentParts.join("\n\n---\n\n");
  console.log("Final assembled content:", assembledContent);
    ...
    if (cursor) {
      ...
      $(".reference-blocks-list .select-content-item").on("change", function() {
        ...
        if ($(this).prop("checked")) {
          ...
          // Instead of getting content from UI, store just the ID and other metadata
          // The actual content will be fetched from the database during assembly
          selectedContentItems.push({
            id: blockId,
            type: "block",
            title: blockTitle,
            standard: referenceId  // We know this because we're in loadBlocksForReference
          });
          
          console.log(`Added reference block ${blockId} to selected items`);
        } else {
          // Remove from selected items
          selectedContentItems = selectedContentItems.filter(item => 
      ...
      if ($(this).prop("checked")) {
        ...
        if (itemType === "block") {
          ...
          selectedContentItems.push({
            ...
            tags: item.data("tags"),
            standard: item.data("standard"),
            stdLevels: item.data("std-levels"),
          ...
          selectedContentItems.push({
            ...
            content: item.data("content"),
            tags: item.data("tags"),
            refBlocks: item.data("ref-blocks")
          });
        }
      } else {
        // Remove from selected items
...
function runActionWithContent(actionId, content) {
  ...
  store.get(parseInt(actionId)).onsuccess = function(e) {
    ...
    // get conbined content from selected items - already in content variable
